...
_WORKFLOW_CANARY_METADATA_DIR_PREFIX = "go-_workflow-canary_-"
...
def _resolve_workflow_canary_context(
    ...
    *,
    ...
    trigger = "@go read"
    trigger_end = len(trigger)
    if (
        not requirement.startswith(trigger)
        or len(requirement) <= trigger_end
        or not requirement[trigger_end].isspace()
        or not requirement[trigger_end:].strip()
    ):
    ...
    if is_inside(runtime_root, project_root):
        raise WorkflowCanaryError(
            "workflow canary runtime directory must stay outside the project"
        )
...
def run_go_dispatch(
    ...
    *,
    ...
    workflow_canary_contracts_dir: Path | None = None
    if workflow_canary:
        ...
        project_contract = (
            ...
            / "rules"
            / "project-contracts"
            / f"{resolved_project_id}.md"
        )
        workflow_canary_contracts_dir = (
            project_contract.parent
            if project_contract.is_file()
            else runtime_root / "workflow-canary-contracts"
        )
        target_shards = [[]]
        provider_id = ""
    ...
    for index in range(agents):
        ...
        dispatch_result = rdgoal(
            ...
            work_type="coding",
            workflow_profile=workflow_profile,
            contracts_dir=workflow_canary_contracts_dir,
        )
    ...
    run_id_prefix = (
        _WORKFLOW_CANARY_METADATA_DIR_PREFIX
        if workflow_canary
        else "go-"
    )
    go_run_id = (
        f"{run_id_prefix}{project_id or project_root.name}-"
...
def load_go_run_result(runtime_dir: str | Path, run_id: str = "latest") -> GoDispatchResult:
    ...
    data = _read_go_run_metadata(metadata_path)
    return _go_result_from_metadata(
        data,
        fallback_runtime_dir=runtime_root,
...
def load_go_run_result_snapshot(
    ...
    result = _go_result_from_metadata(
        data,
        fallback_runtime_dir=runtime_root,
        metadata_path=metadata_path,
    )
...
def _go_result_from_metadata(
    data: dict[str, Any],
    *,
    fallback_runtime_dir: Path,
    metadata_path: Path,
) -> GoDispatchResult:
    workflow_canary_provenance = _workflow_canary_metadata_provenance(
        data,
        metadata_path,
    ...
    workflow_canary_envelope_validated = _validate_workflow_canary_top_level_envelope(
        data,
        require_canary=workflow_canary_provenance,
    )
...
def _workflow_canary_metadata_provenance(
    data: dict[str, Any],
    metadata_path: Path,
    runtime_root: Path,
) -> bool:
    path_is_canary = _is_workflow_canary_metadata_path(metadata_path)
    marker_present = "workflow_canary" in data
    persisted_run_id = data.get("go_run_id")
    path_run_id = metadata_path.parent.name
    run_id_matches = isinstance(persisted_run_id, str) and (
        persisted_run_id.casefold() == path_run_id.casefold()
        if sys.platform == "win32"
        else persisted_run_id == path_run_id
    )
    if not run_id_matches:
        if path_is_canary or marker_present:
            raise WorkflowCanaryError(
                "workflow canary persisted run id does not match its metadata path"
            )
        raise ValueError("go run metadata id does not match its metadata path")
    if marker_present and not path_is_canary:
        raise WorkflowCanaryError(
            "workflow canary marker is forbidden outside its reserved metadata path"
        )
    if path_is_canary:
        _validate_workflow_canary_metadata_location(metadata_path, runtime_root)
    return path_is_canary


def _validate_workflow_canary_metadata_location(
    metadata_path: Path,
    runtime_root: Path,
) -> None:
    go_runs_root = runtime_root / "go-runs"
    try:
        resolved_metadata_path = metadata_path.resolve(strict=True)
        resolved_go_runs_root = go_runs_root.resolve(strict=True)
        has_link_or_junction = any(
            _is_link_or_junction(path)
            for path in (metadata_path, metadata_path.parent, go_runs_root)
        )
    except OSError as exc:
        raise WorkflowCanaryError(
            "workflow canary metadata path is unreadable"
        ) from exc
    if (
        has_link_or_junction
        or not is_inside(resolved_metadata_path, resolved_go_runs_root)
    ):
        raise WorkflowCanaryError(
            "workflow canary metadata path must stay in its physical runtime directory"
        )
...
def _is_workflow_canary_metadata_path(path: Path) -> bool:
    return path.parent.name.casefold().startswith(
        _WORKFLOW_CANARY_METADATA_DIR_PREFIX.casefold()
    )

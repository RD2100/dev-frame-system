...
def _paper_run_id(project_path: str | Path) -> str:
    ...
    if not paper_id or "{{" in paper_id or "}}" in paper_id:
        raise ClusterRunError("invalid paper project: paper_id is missing or unrendered")
    normalized = "".join(
        char.lower() if char.isascii() and char.isalnum() else "-"
        for char in paper_id
    )
...
def _validate_paper_project(project_path: str | Path) -> str:
    ...
    if any((root / name).is_symlink() for name in _PAPER_PROJECT_FILES):
        raise ClusterRunError("invalid paper project: initialized files must not be symlinks")
    ...
    if input_dir.exists():
        if input_dir.is_symlink() or not input_dir.is_dir():
            raise ClusterRunError("unsafe paper project: input must be a real directory")
        unexpected = sorted(
            relative
...
def _verify_paper_execution(root: Path, results: list[Any]) -> list[str]:
    ...
    failed_stages = [result.stage_id for result in results if result.status != "completed"]
    ...
    evidence_refs = [str((root / relative).resolve()) for relative in _PAPER_REQUIRED_EVIDENCE]
    missing_evidence = [path for path in evidence_refs if not Path(path).is_file()]
    if missing_evidence:
        raise ClusterRunError(
            "paper pipeline evidence is missing: " + ", ".join(missing_evidence)
        )
...
def _paper_review_action(
    ...
    paper_action = next(
        (
            ...
            if item.get("detail") == "rdpaper"
            and item.get("source_id") == paper_run_id
        ),
        None,
    )
    if not paper_run or paper_run.get("status") != "review_required":
        raise ClusterRunError("paper review-pending run is not visible in the control plane")
...
def _run_paper_workflow(
    ...
    logger.debug("Offline paper pipeline reached review_pending: paper_run_id=%s", paper_run_id)
    return {
        "kind": "paper",
        "status": "review_pending",
        "summary": "Paper pipeline completed; independent review and explicit finalization are required.",

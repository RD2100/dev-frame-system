...
def _render_prompt(review_question: str, sources: list[dict[str, Any]], status: str, issues: list[str]) -> str:
    lines = [
        ...
        "",
        "Conditional navigation single-flight review (apply only when the reviewed handler keeps a busy or token lock across navigation; otherwise skip this section):",
        "- Distinguish the `navigateTo`/`navigateBack` return lifecycle from the `redirectTo`/`switchTab`/`reLaunch` unload lifecycle.",
        "- If busy or token remains held after a successful `navigateTo`, require a synchronous double-trigger probe, a success -> `onHide` -> `onShow` -> retry probe, a failure -> retry probe, and an unload-stale probe.",
        "- Lifecycle methods must release the return lock; direct test-only state mutation is insufficient.",
        "",

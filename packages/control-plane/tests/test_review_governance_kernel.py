...
# ---- Phase 1B: full projection consistency ----


def test_projection_fabricated_allowed_actions_fails():
    payload = _load_fixture("blocked.json")
    payload["projection"]["allowed_actions"] = ["view", "execute", "edit"]
    result = validate_packet(payload)
    assert not result.valid
    assert any("allowed_actions" in e for e in result.errors)


def test_projection_fabricated_evidence_summary_fails():
    payload = _load_fixture("success.json")
    payload["projection"]["evidence_summary"] = {
        "total_evidence_count": 999,
        "supporting_count": 999,
        "rejecting_count": 999,
        "inconclusive_count": 999,
    }
    result = validate_packet(payload)
    assert not result.valid
    assert any("evidence_summary" in e for e in result.errors)


def test_projection_stale_latest_decision_id_fails():
    payload = _load_fixture("success.json")
    # Add a new decision that supersedes the original latest
    payload["decisions"].append({
        "id": "decision-gate-later",
        "project_id": "proj-review-demo",
        "kind": "gate",
        "target_ref": "wi-review-1",
        "decider_principal_id": "principal-human-1",
        "outcome": "blocked",
        "evidence_ids": ["ev-1"],
        "rationale": "later gate blocks",
        "payload": {"blocked_reasons": ["late gate"]},
    })
    # Keep old latest_decision_id in projection (stale)
    result = validate_packet(payload)
    assert not result.valid
    assert any("latest_decision_id" in e for e in result.errors)


def test_projection_stale_gate_outcome_fails():
    payload = _load_fixture("success.json")
    # Add a later gate decision that blocks
    payload["decisions"].append({
        "id": "decision-gate-later-blocked",
        "project_id": "proj-review-demo",
        "kind": "gate",
        "target_ref": "wi-review-1",
        "decider_principal_id": "principal-human-1",
    ...
    result = validate_packet(payload)
    assert not result.valid
    assert any("gate_outcome" in e for e in result.errors)


def test_missing_context_fixture_matches_derive():
    payload = _load_fixture("missing-context.json")

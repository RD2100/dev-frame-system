...
def _run_generated_bridge_goal(
    ...
    *,
    ...
    probe.write_text(
        ...
        "let result;\n"
        "try {\n"
        "  const value = await startDevFrameCoordinatorGoal(config, {\n"
        ...
        "      status: typeof error === 'object' && error !== null && 'status' in error ? error.status : 0,\n"
        "      code: typeof error === 'object' && error !== null && 'code' in error ? error.code : '',\n"
        "      detail: typeof error === 'object' && error !== null && 'detail' in error ? error.detail : '',\n"
        "    },\n"
        "  };\n"
        "}\n"
...
def test_generated_rdcode_bridge_actual_dashboard_workflow_and_worker_contain_secret(
    ...
    try:
        rejected = _run_generated_bridge_goal(
            t3_root,
            base_url=base_url,
            project=project,
        )
        ...
        assert error["code"] == "cluster_run_rejected"
        assert "missing_external_secret" in str(error["detail"])
        assert not marker.exists()
        assert not [path for path in runtime.rglob("*") if path.is_file()]
        assert list_cluster_runs(runtime) == []

        monkeypatch.setenv(SECRET_ENV, secret)
        accepted = _run_generated_bridge_goal(
            t3_root,
            base_url=base_url,
            project=project,
        )

...
| **Rule** | The canonical project root is `<repo-root>`. All path references in execution reports, evidence chains, and contract records must resolve to absolute paths rooted at this directory. Relative paths are resolved from this root. |
...
| **Detection** | Grep all output files for relative path references (not starting with `<repo-root>\`). Verify `git rev-parse --show-toplevel` equals the canonical root. |
...
| **Violation Example** | A contract document references `<wrong-ai-workflow-hub-root>` when the actual path is `<ai-workflow-hub-root>`. An invariant references `<other-user-home>` when the actual home is `<user-home>`. |
...
| **Rule** | Agents may only write to paths explicitly approved in the batch plan's "Approved Output Path" section. No write to any other path is permitted. The approved scope for Batch D2 is: `<repo-root>\docs\agent-runtime\runtime-invariants.md`. |
| **Violation Example** | An agent writes `<repo-root>\docs\FLOW_CATALOG.md` (a dirty baseline file not in approved scope). An agent creates `<repo-root>\new-file.log` without batch plan approval. |
...
| **Priority** | P0 (Hard Stop) |
| **Scope** | Phase 0-5 |
| **Rule** | Any new file created must be under a directory path explicitly approved in the batch plan. For Batch D2, only `<repo-root>\docs\agent-runtime\` is approved for new file creation. No files may be created at the repository root or in unapproved subdirectories. |
| **Violation Example** | An agent creates `<repo-root>\templates\new-template.md` without approval. An agent creates `<repo-root>\invariants-backup.md` at the repo root. |
...
| **Violation Example** | Pre-batch shows `M README.md`. Post-batch also shows `M README.md` but the content hash differs -- it was silently edited. A new untracked file `<repo-root>\test.log` appears that is not in the batch-approved output paths. |
...
| **Priority** | P0 (Hard Stop) |
| **Scope** | Phase 0-5 |
| **Rule** | Agents must not write to any memory file. This includes `memory/*.md`, `MEMORY.md` index, `ACTIVE.md`, `agent-state.db`, or any file within the user's local agent configuration directory (for example, `<user-home>\.claude\`). Memory reads are permitted. Memory writes require `MemoryUpdateRecord` with status `proposed` only. |
...
| **Violation Example** | An agent runs `Read` on `<repo-root>\.env` to check configuration. An agent greps for `*.pem` files and reads their contents. |
...
| **Detection** | For any user-supplied path, resolve the canonical path and verify it starts with `<repo-root>\` or the explicitly approved output directory. |
...
| **Priority** | P1 (Scope Control) |
| **Scope** | All phases |
| **Rule** | All `artifact_path` values in EvidenceIndex records must be within the project root (`<repo-root>\`). Path traversal outside the project root is a P0 violation (see INV-037). Even internal paths must be validated to prevent drift. |

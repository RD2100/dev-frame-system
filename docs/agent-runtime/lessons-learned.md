...
- **Changed Files**: `dispatch-model-profiles.md`, SADP section 4.6
...
- **Problem**: `opencode run --add-dir <agent-acceptance-root>` -> exit 1, shows help
...
- **Changed Files**: `sub-agent-dispatch-protocol.md` section 2.1, section 4.6
...
- **Context**: User proposed a generalized resource routing/invocation capability. Legacy corrupted quote rewritten from context. Agent agreed and began designing new Resource Activation Framework.
...
- **Derived Pattern**: `Prompt-Induced Redundant Construction` -> when user instruction implies "build X", agent defaults to compliance. Legacy corrupted trigger list rewritten from context as categories: build/create/refactor/generalize/import/integrate.
- **Prevention**: Gate 0 must run before TaskSpec creation. Trigger categories -> auto-trigger Gate 0.
- **Changed Files**: `rules/core.md` (+core-008), `sub-agent-dispatch-protocol.md` (section 0), `CLAUDE.md`, `lessons-learned.md` (LL-007)
...
- **Prevention**:
  ...
  3. Add "cascade check" to Plan Agent Review Procedure (section 3.3a) regression tests
...
- **Root Cause**:
  ...
  3. SADP section 0.0 auto-trigger rules are DOCUMENT DECLARATIONS, not ENFORCEMENT MECHANISMS -> nothing stops a plan agent from executing directly
...
- **Structural Gap Exposed**:
  - SADP section 0.0 says "Plan agent violation: ... human reviewer should reject the output." But there is no automated enforcement.
...
- **Prevention Candidates** (not yet implemented):
  ...
  3. Cumulative trigger window (section 0.0a) partially addresses this by tracking session-level write_set -> but still relies on agent self-reporting

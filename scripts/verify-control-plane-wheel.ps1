...
try {
    ...
    Invoke-Step "devframe code help" $python @(
        "-c",
        "import subprocess, sys; text = subprocess.check_output([sys.argv[1], 'code', '--help'], text=True); assert 'Usage: devframe code ' in text; assert '<goal>' in text; assert '--changed' in text; assert '--agents' in text; assert '--max-agents' in text; assert '--preview' in text; assert '--worker' in text; assert '--dashboard' not in text; assert 'devframe dashboard serve' in text; print('devframe code help ok')",
        $devframe
    )
...
'@
    ...
    Invoke-Step "devframe code dry dispatch details" $python @(
        "-c",
        "import json, pathlib, subprocess, sys; root = pathlib.Path(sys.argv[2]); files = list((root / 'go-runs').glob('*/go-run.json')); assert len(files) == 1; data = json.loads(files[0].read_text(encoding='utf-8')); assert data['status'] == 'queued'; assert len(data['agents']) == 1; assert data['agents'][0]['targets'] == ['CURRENT_STATE.yaml']; profile = data['workflow_profile']; assert profile['profile_id'] == 'governed-coding-v1'; assert profile['execution_state'] == 'planned_only'; task = json.loads(pathlib.Path(data['agents'][0]['task_spec_path']).read_text(encoding='utf-8')); assert task['work_type'] == 'coding'; assert task['workflow_profile']['profile_fingerprint'] == profile['profile_fingerprint']; assert 'skill_usage' not in task; text = subprocess.check_output([sys.argv[1], 'visual-state', '--runtime-dir', sys.argv[2]], text=True); state = json.loads(text); assert len(state['go_runs']) == 1; assert len(state['go_runs'][0]['agents']) == 1; assert len(state['runs']) == 1; print('code dispatch ok')",
        $devframe,
        $codeRuntimeDir
    )

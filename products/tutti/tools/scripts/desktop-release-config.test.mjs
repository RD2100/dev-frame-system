...
const windowsBuildScriptPath = new URL(
  "../../tools/scripts/build-desktop-package-win.mjs",
  import.meta.url
);
...
test("desktop windows packaging anchors electron-builder workspace detection to the repo root", async () => {
  const packageJson = JSON.parse(await readFile(desktopPackagePath, "utf8"));
  const buildScript = await readFile(windowsBuildScriptPath, "utf8");

  assert.equal(
    packageJson.scripts["build:win"],
    "node ../../tools/scripts/build-desktop-package-win.mjs"
  );
  assert.equal(
    packageJson.scripts["build:win:preview"],
    "node ../../tools/scripts/build-desktop-package-win.mjs --preview"
  );
  assert.match(
    buildScript,
    /npm_package_json:\s*join\(workspaceRoot, "package\.json"\)/
  );
  assert.match(buildScript, /INIT_CWD:\s*workspaceRoot/);
  assert.match(buildScript, /tuttid\.exe/);
  assert.match(buildScript, /tutti\.exe/);
  assert.match(buildScript, /-c\.win\.signAndEditExecutable=false/);
});

...
func TestOpenCodeAdapterResolvesCommandFromProviderResolver(t *testing.T) {
	t.Parallel()

	resolvedCommand := []string{"C:\\Users\\RD\\.local\\bin\\node_modules\\opencode-ai\\bin\\opencode.exe", "acp"}
	resolvedEnv := []string{"OPENCODE_CONFIG_DIR=C:\\Users\\RD\\.config\\opencode"}
	resolver := func(_ context.Context, provider string) (ProviderCommand, error) {
		if provider != ProviderOpenCode {
			t.Errorf("resolver provider = %q, want %q", provider, ProviderOpenCode)
		}
		return ProviderCommand{Command: resolvedCommand, Env: resolvedEnv}, nil
	}

	transport := newStandardACPTransport("OpenCode", "opencode-session-cmd-resolver")
	adapter := NewOpenCodeAdapterWithHostMetadata(transport, LegacyHostMetadata(), resolver)
	...
	if len(transport.specs) != 1 {
		t.Fatalf("specs count = %d, want 1", len(transport.specs))
	}
	spec := transport.specs[0]
	if len(spec.Command) != len(resolvedCommand) || spec.Command[0] != resolvedCommand[0] || spec.Command[1] != resolvedCommand[1] {
		t.Fatalf("spec.Command = %#v, want %#v", spec.Command, resolvedCommand)
	}
	found := false
	...
	if !found {
		t.Fatalf("spec.Env missing resolver env %q", resolvedEnv[0])
	}
}

...
`build:win:preview` produces a local unpacked, unsigned directory build.
`build:win` runs the full packaging path and attempts installer generation
and code signing, propagating any failure. Installer generation and code
signing are not yet supported or verified by the current Windows packaging
spike; the normal `build:win` path may fail until those parts of the

...
### Windows workspace app fails to start with "bootstrap.ps1 sibling" error

- Symptom:
  On Windows, a workspace app fails to start and runtime.log shows
  `windows workspace app bootstrap "bootstrap.sh" is a POSIX shell script;
  add a bootstrap.ps1 sibling beside bootstrap.sh`.
- Quick checks:
  Verify the app package contains `bootstrap.ps1` beside `bootstrap.sh`.
  The manifest field stays `"bootstrap": "bootstrap.sh"`; the runner picks
  the `.ps1` sibling automatically on Windows.
- Root cause:
  On Windows, `.sh` files cannot be executed directly. The runner requires
  a PowerShell `.ps1` bootstrap on Windows instead of the POSIX shell script.
- Fix:
  Add a `bootstrap.ps1` sibling file that reads `TUTTI_APP_*` environment
  variables and launches the app server. Use `powershell.exe -File` to test
  locally (see [Workspace App Runtime](./workspace-app-runtime.md)).
- Validation:
  ```powershell
  powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .\bootstrap.ps1
  ```
- References:
  [apps_runner_bootstrap.go](../../services/tuttid/service/workspace/apps_runner_bootstrap.go)
  [workspace-app-runtime.md](./workspace-app-runtime.md)

### Windows workspace app launch fails with "unsupported extension" for .cmd/.bat

- Symptom:
  `windows workspace app bootstrap "bootstrap.cmd" uses an unsupported extension`.
- Quick checks:
  Rename the bootstrap to `.ps1` (PowerShell) or `.exe` (native binary).
  `.cmd` and `.bat` files are not supported as workspace app bootstraps.
- Root cause:
  The runner deliberately rejects `.cmd` and `.bat` to avoid shell-quoting
  risks with `cmd.exe`. Use PowerShell `.ps1` instead.
- Fix:
  Convert the `.cmd`/`.bat` script to a `.ps1` PowerShell script, or use an
  `.exe` bootstrap if the app is a native binary.
- References:
  [apps_runner_bootstrap.go](../../services/tuttid/service/workspace/apps_runner_bootstrap.go)

...
function copyAppRuntimeForWindows(context, resourcesDir) {
  if (context.electronPlatformName !== "win32") {
    return;
  }
  const source = join(desktopDir, "build", "app-runtime");
  if (!existsSync(source)) {
    throw new Error(`vendored app-runtime missing: ${source}`);
  }
  const destination = join(resourcesDir, "app-runtime");
  rmSync(destination, { recursive: true, force: true });
  mkdirSync(dirname(destination), { recursive: true });
  cpSync(source, destination, { recursive: true, dereference: true });
  log(`copied app-runtime to ${destination}`);
}
...
export default async function copyVendoredNodeResourcesAfterPack(context) {
  ...
  copyAppRuntimeForWindows(context, resourcesDir);
}

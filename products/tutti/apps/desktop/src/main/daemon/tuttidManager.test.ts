...
test("resolveManagedRuntimeDaemonEnv sets vendored root for packaged Windows", async () => {
  const previousEnv = { ...process.env };
  try {
    delete process.env.TUTTI_APP_RUNTIME_ROOT;
    delete process.env.TUTTI_APP_RUNTIME_CACHE_ROOT;
    const resourcesPath = await mkdtemp(
      join(tmpdir(), "tutti-resources-")
    );
    const nodeBinDir = join(resourcesPath, "app-runtime", "node", "bin");
    await mkdir(nodeBinDir, { recursive: true });
    await writeFile(join(nodeBinDir, "node.exe"), "stub");
    await writeFile(join(nodeBinDir, "npm.cmd"), "stub");
    const npmCliDir = join(nodeBinDir, "node_modules", "npm", "bin");
    await mkdir(npmCliDir, { recursive: true });
    await writeFile(join(npmCliDir, "npm-cli.js"), "stub");
    await writeFile(
      join(resourcesPath, "app-runtime", "node", "LICENSE"),
      "stub"
    );
    await writeFile(
      join(resourcesPath, "app-runtime", "node", ".tutti-runtime-metadata.json"),
      JSON.stringify({
    ...
    const got = resolveManagedRuntimeDaemonEnv(
      {},
      { isPackaged: true, resourcesPath },
      "win32"
    );
    assert.equal(
      got.TUTTI_APP_RUNTIME_ROOT,
      join(resourcesPath, "app-runtime")
    );
  } finally {
    restoreEnv(previousEnv);
  }
...
test("resolveManagedRuntimeDaemonEnv falls back when vendored root is missing on Windows", async () => {
  const previousEnv = { ...process.env };
  try {
    delete process.env.TUTTI_APP_RUNTIME_ROOT;
    delete process.env.TUTTI_APP_RUNTIME_CACHE_ROOT;
    const resourcesPath = await mkdtemp(
      join(tmpdir(), "tutti-resources-missing-")
    );
...
test("resolveManagedRuntimeDaemonEnv falls back for corrupt root on Windows", async () => {
  const previousEnv = { ...process.env };
  try {
    delete process.env.TUTTI_APP_RUNTIME_ROOT;
    delete process.env.TUTTI_APP_RUNTIME_CACHE_ROOT;
    const resourcesPath = await mkdtemp(
      join(tmpdir(), "tutti-resources-corrupt-")
    );
    await mkdir(
      join(resourcesPath, "app-runtime", "node", "bin"),
      { recursive: true }
    );
...
test("resolveManagedRuntimeDaemonEnv honors explicit TUTTI_APP_RUNTIME_ROOT override", () => {
  const previousEnv = { ...process.env };
  try {
    process.env.TUTTI_APP_RUNTIME_ROOT = "/custom/runtime-root";
    delete process.env.TUTTI_APP_RUNTIME_CACHE_ROOT;
    const got = resolveManagedRuntimeDaemonEnv(
      {},
      { isPackaged: true, resourcesPath: join(tmpdir(), "tutti-resources") },
      "win32"
    );
    assert.deepEqual(got, {});
  } finally {
...
test("resolveManagedRuntimeDaemonEnv honors explicit TUTTI_APP_RUNTIME_CACHE_ROOT override", () => {
  const previousEnv = { ...process.env };
  try {
    delete process.env.TUTTI_APP_RUNTIME_ROOT;
    process.env.TUTTI_APP_RUNTIME_CACHE_ROOT = "/custom/cache-root";
    const got = resolveManagedRuntimeDaemonEnv(
      {},
      { isPackaged: true, resourcesPath: join(tmpdir(), "tutti-resources") },
      "win32"
...
test("resolveManagedRuntimeDaemonEnv uses cache root in development", () => {
  const previousEnv = { ...process.env };
  try {
    delete process.env.TUTTI_APP_RUNTIME_ROOT;
    delete process.env.TUTTI_APP_RUNTIME_CACHE_ROOT;
    const got = resolveManagedRuntimeDaemonEnv(
      {},
      { isPackaged: false, resourcesPath: join(tmpdir(), "tutti-resources") },
      "win32"
    );
    assert.equal(
      got.TUTTI_APP_RUNTIME_CACHE_ROOT?.endsWith("app-runtimes"),
...
test("resolveManagedRuntimeDaemonEnv uses cache root on non-Windows platform", () => {
  const previousEnv = { ...process.env };
  try {
    delete process.env.TUTTI_APP_RUNTIME_ROOT;
    delete process.env.TUTTI_APP_RUNTIME_CACHE_ROOT;
...
test("isWindowsNodeRuntimeRootValid rejects directory as node.exe", async () => {
  const root = await mkdtemp(join(tmpdir(), "tutti-validate-"));
  try {
    const appRTDir = join(root, "app-runtime");
    const nodeBinDir = join(appRTDir, "node", "bin");
    await mkdir(nodeBinDir, { recursive: true });
    await mkdir(join(nodeBinDir, "node.exe"), { recursive: true });
    await writeFile(join(nodeBinDir, "npm.cmd"), "stub");
    const npmCliDir = join(nodeBinDir, "node_modules", "npm", "bin");
    await mkdir(npmCliDir, { recursive: true });
    await writeFile(join(npmCliDir, "npm-cli.js"), "stub");
    await writeFile(join(appRTDir, "node", "LICENSE"), "stub");
    await writeFile(
    ...
    assert.equal(isWindowsNodeRuntimeRootValid(appRTDir), false);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test("isWindowsNodeRuntimeRootValid rejects stale metadata schema", async () => {
  const root = await mkdtemp(join(tmpdir(), "tutti-validate-"));
  try {
    const appRTDir = join(root, "app-runtime");
    const nodeBinDir = join(appRTDir, "node", "bin");
...
test("isWindowsNodeRuntimeRootValid accepts valid metadata", async () => {
  const root = await mkdtemp(join(tmpdir(), "tutti-validate-"));
  try {
    const appRTDir = join(root, "app-runtime");
    const nodeBinDir = join(appRTDir, "node", "bin");
    ...
    assert.equal(isWindowsNodeRuntimeRootValid(appRTDir), true);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test("isWindowsNodeRuntimeRootValid rejects wrong node version", async () => {
  const root = await mkdtemp(join(tmpdir(), "tutti-validate-"));
  try {
    const appRTDir = join(root, "app-runtime");
    const nodeBinDir = join(appRTDir, "node", "bin");

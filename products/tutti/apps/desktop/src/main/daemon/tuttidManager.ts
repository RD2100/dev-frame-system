...
import { pinnedWindowsNodeVersion } from "./windowsNodeRuntimeVersion.ts";
...
const windowsNodeRuntimeMetadataSchema = "tutti.windows-node-runtime-metadata.v1";

export function isWindowsNodeRuntimeRootValid(root: string): boolean {
  if (!root || !existsSync(root)) {
    return false;
  }
  if (!pinnedWindowsNodeVersion) {
    return false;
  }
  const metaPath = join(root, "node", ".tutti-runtime-metadata.json");
  try {
    ...
    if (
      !meta ||
      typeof meta !== "object" ||
      meta.schemaVersion !== windowsNodeRuntimeMetadataSchema ||
      typeof meta.nodeVersion !== "string" ||
      meta.nodeVersion.length === 0
    ) {
      return false;
    }
    if (meta.nodeVersion !== pinnedWindowsNodeVersion) {
      return false;
    }
  } catch {
    return false;
  ...
  return (
    isRegularFile(join(binDir, "node.exe")) &&
    isRegularFile(join(binDir, "npm.cmd")) &&
    isRegularFile(
      join(binDir, "node_modules", "npm", "bin", "npm-cli.js")
    ) &&
    isRegularFile(join(root, "node", "LICENSE"))
  );
}
...
export function resolveManagedRuntimeDaemonEnv(
  userShellEnv?: Record<string, string>,
  runtime?: DesktopElectronAppRuntime,
  platform?: string
): Record<string, string> {
  const winPlatform = platform ?? process.platform;
  const rootOverride =
  ...
  if (winPlatform === "win32") {
    ...
    if (appRuntime.isPackaged) {
      const vendoredRoot = join(appRuntime.resourcesPath, "app-runtime");
      if (isWindowsNodeRuntimeRootValid(vendoredRoot)) {
        return { TUTTI_APP_RUNTIME_ROOT: vendoredRoot };
      }
    }
  }
  return {

...
async function packageBuiltin({ checkOnly = false } = {}) {
  ...
  return await withPackageLock(async () => {
    ...
    try {
      await writePackageZip(tempZipPath);
...
async function runViteBuild() {
  if (process.env.npm_execpath) {
    await run(
      process.execPath,
      [process.env.npm_execpath, "exec", "vite", "build"],
      { cwd: appDir }
    );
    return;
  }
  if (process.platform === "win32") {
    await run(
      ...
      ["/d", "/s", "/c", "pnpm.cmd exec vite build"],
      { cwd: appDir }
    );
    return;
  }
...
async function validatePackageRoot(root) {
  ...
  if (!bootstrapStat.isFile()) {
    throw new Error("bootstrap.sh must be a file.");
  }
...
async function writePackageZip(zipPath) {
  ...
  for (const filePath of await listPackageFiles(packageRoot)) {
    const relativePath = path
      .relative(packageRoot, filePath)
      .replaceAll("\\", "/");
    zip.addFile(filePath, relativePath, {
      mode: relativePath === "bootstrap.sh" ? 0o100755 : 0o100644
    });
  }
  zip.end();
  await completed;
...
async function listPackageFiles(root) {
  ...
  for (const entry of await readdir(root, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      files.push(...(await listPackageFiles(entryPath)));
      continue;
    }
    if (entry.isFile()) {

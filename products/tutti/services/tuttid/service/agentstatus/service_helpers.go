...
func runDefaultInstallCommand(ctx context.Context, input InstallCommandInput) (InstallCommandResult, error) {
	ctx = baseContext(ctx)
	if len(input.Args) > 0 {
		return runArgvInstallCommand(ctx, input)
	}
...
func runArgvInstallCommand(ctx context.Context, input InstallCommandInput) (InstallCommandResult, error) {
	if len(input.Args) == 0 {
		return InstallCommandResult{ExitCode: 1}, errors.New("installer args are empty")
	}

	exe := input.Args[0]

	if runtime.GOOS == "windows" && isBatchFile(exe) {
		return InstallCommandResult{ExitCode: 1,
			Stderr: fmt.Sprintf("batch file %q cannot be executed directly; resolve the native entry point instead", exe)}, nil
	}

	cmd := exec.CommandContext(ctx, exe, input.Args[1:]...)
	cmd.Dir = strings.TrimSpace(input.CWD)
	cmd.Env = input.Env
	var stdout bytes.Buffer
	var stderr bytes.Buffer

...
func TestDefaultRegistryUsesOpenCodeManagedNPMInstallerOnWindows(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Windows-specific OpenCode installer spec test")
	}
	specs, err := DefaultRegistry().Select([]string{"opencode"})
	if err != nil {
		t.Fatalf("Select() error = %v", err)
	}
	if len(specs) != 1 {
	...
	if install.ManagedNPM.PackageName != "opencode-ai" {
		t.Fatalf("PackageName = %q, want opencode-ai", install.ManagedNPM.PackageName)
	}
	if install.ManagedNPM.BinaryName != "opencode" {
		t.Fatalf("BinaryName = %q, want opencode", install.ManagedNPM.BinaryName)
	}
	if !install.ManagedNPM.IncludeOptional {
		t.Fatalf("IncludeOptional = false, want true")
	}
...
func TestDefaultRegistryUsesOpenCodeOfficialScriptOnNonWindows(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("Non-Windows OpenCode installer spec test")
	}
	specs, err := DefaultRegistry().Select([]string{"opencode"})
	if err != nil {
		t.Fatalf("Select() error = %v", err)
	...
	if install.ScriptURL != "https://opencode.ai/install" {
		t.Fatalf("ScriptURL = %q, want https://opencode.ai/install", install.ScriptURL)
	}
	if install.ScriptShell != "bash" {
		t.Fatalf("ScriptShell = %q, want bash", install.ScriptShell)
	}
}

func TestOpenCodeInstallerSpecIsPlatformAware(t *testing.T) {
	spec := opencodeInstallerSpec()
	if runtime.GOOS == "windows" {
		if spec.Kind != InstallerKindManagedNPMPackage {
			t.Fatalf("Kind = %q, want %q", spec.Kind, InstallerKindManagedNPMPackage)
		}
		if spec.ManagedNPM == nil || spec.ManagedNPM.PackageName != "opencode-ai" {
			t.Fatalf("ManagedNPM = %#v, want opencode-ai", spec.ManagedNPM)
		}
	} else {
		if spec.Kind != InstallerKindOfficialScript {
			t.Fatalf("Kind = %q, want %q", spec.Kind, InstallerKindOfficialScript)
		}
		if spec.ScriptURL != "https://opencode.ai/install" {
			t.Fatalf("ScriptURL = %q, want https://opencode.ai/install", spec.ScriptURL)
		}
	}
}

func TestOpenCodeManagedNPMInstallerRunsWithManagedRuntimeOnWindows(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Windows-specific OpenCode managed npm install test")
	}
	home := t.TempDir()
	...
	if _, err := service.runManagedNPMPackageInstaller(context.Background(), "opencode", ManagedNPMPackageInstallerSpec{
	...
	if !strings.Contains(command.Command, managedNPM) ||
		...
		!strings.Contains(command.Command, "opencode-ai") ||
...
func TestResolveProviderRuntimeFindsOpenCodeNativeExeOnWindows(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Windows-specific OpenCode native exe resolution test")
	}
	...
	runtime := service.resolveProviderRuntime(context.Background(), spec)
	if runtime.CLIPath != exePath {
		t.Fatalf("CLIPath = %q, want %q", runtime.CLIPath, exePath)
	}
	if runtime.AdapterPath != exePath {
		t.Fatalf("AdapterPath = %q, want %q", runtime.AdapterPath, exePath)
	}
}

func TestResolveStaticProviderSpecResolvesOpenCodeAdapterCommandOnWindows(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Windows-specific OpenCode adapter command resolution test")
	}
	...
	specs, err := service.selectProviderSpecs(context.Background(), []string{"opencode"}, false)
	if err != nil {
	...
	cmd := specs[0].AdapterCommand
	if len(cmd) < 2 {
		t.Fatalf("AdapterCommand len = %d, want >= 2", len(cmd))
	}
	if cmd[0] != exePath {
		t.Fatalf("AdapterCommand[0] = %q, want %q", cmd[0], exePath)
	}
	if cmd[1] != "acp" {
		t.Fatalf("AdapterCommand[1] = %q, want acp", cmd[1])
	}
}

func TestResolveStaticProviderSpecNoopForCodexOnWindows(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Windows-specific Codex adapter command noop test")
	}
	...
	service.HomeDir = func() (string, error) { return home, nil }
	service.Environ = func() []string {
		return []string{"PATH=C:\\Windows\\System32"}
	}
	service.IsExecutableFile = isTestExecutableUnderHome(home)

	specs, err := service.selectProviderSpecs(context.Background(), []string{"codex"}, false)
	if err != nil {
		t.Fatalf("selectProviderSpecs() error = %v", err)
	}
	if len(specs) != 1 {
	...
	if len(cmd) == 0 || cmd[0] != "codex" {
		t.Fatalf("AdapterCommand[0] = %q, want codex (unchanged for non-managed-NPM provider)", cmd[0])
	}
}
...
func TestServiceRunActionSerializesConcurrentExternalRegistryNPMInstalls(t *testing.T) {
	...
	select {
	case <-firstInstallStarted:
	case <-time.After(15 * time.Second):
		t.Fatalf("first install did not start within timeout; calls=%d", npmInstallCalls.Load())
	}
	...
	case <-time.After(15 * time.Second):
		t.Fatalf("first RunAction did not complete within timeout")
	}
	if first.err != nil {
	...
	case <-time.After(15 * time.Second):
		t.Fatalf("second RunAction did not complete within timeout")
	}
	if second.err != nil {
...
func fakeManagedRuntimeRoot(t *testing.T) string {
	...
	nodePath := filepath.Join(nodeBinDir, nodeBinaryNameForTest())
	if realNode, err := exec.LookPath(nodeBinaryNameForTest()); err == nil {
		copyFile(t, realNode, nodePath)
	} else {
		writeExecutable(t, nodePath, "#!/bin/sh\nexit 0\n")
	}
	writeExecutable(t, filepath.Join(nodeBinDir, npmBinaryNameForTest()), "#!/bin/sh\nexit 0\n")
	setupNPMCLIJS(t, nodeBinDir)
	return root
}

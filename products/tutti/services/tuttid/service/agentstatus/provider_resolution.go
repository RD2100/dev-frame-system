...
func (s Service) resolveStaticProviderSpec(ctx context.Context, spec ProviderSpec, requireManagedRuntime bool) ProviderSpec {
	...
	if runtime.GOOS == "windows" && spec.Install.Kind == InstallerKindManagedNPMPackage && spec.Install.ManagedNPM != nil {
		spec = s.resolveManagedNPMAdapterCommand(spec)
	}
...
func (s Service) resolveExternalRegistryNPMSpec(
	...
	if runtime.GOOS == "windows" {
		command = s.resolveExternalRegistryNPMWindowsCommand(appRuntime, prefixDir, packageDir, packageName, distribution)
		if len(command) == 0 {
			spec.AdapterCommand = nil
			spec.AdapterEnv = nil
			spec.AdapterUnavailableReasonCode = ReasonManagedRuntimeUnavailable
...
func (s Service) resolveManagedNodeRuntimeForProvider(ctx context.Context, require bool) (managedruntime.ResolvedRuntime, bool) {
	...
	runtime, err := s.resolveManagedNodeRuntime(ctx)
	if err != nil {
		return managedruntime.ResolvedRuntime{}, false
	}
	return runtime, true
...
func (s Service) resolveExternalRegistryNPMWindowsCommand(
	...
	binJS, binErr := s.installedNPMPackageBinJS(packageDir, packageName)
	if errors.Is(binErr, errBinInvalid) {
		return nil
	}
	if binErr == nil && binJS != "" {
		return []string{appRuntime.Node, binJS}
	}
	npmCLIPath, err := resolveNPMCLIJS(appRuntime.NPM)
	if err != nil {
		return nil
	}
	return []string{
...
func (s Service) installedNPMPackageBinJS(packageDir string, packageName string) (string, error) {
	if strings.TrimSpace(packageDir) == "" || strings.TrimSpace(packageName) == "" {
		return "", errBinNotInstalled
	}
	...
	if err := json.Unmarshal(content, &manifest); err != nil {
		return "", fmt.Errorf("%w: package.json is malformed in %s", errBinInvalid, packageDir)
	}
	if strings.TrimSpace(manifest.Name) != strings.TrimSpace(packageName) {
		return "", fmt.Errorf("%w: package.json name %q does not match expected %q in %s", errBinInvalid, manifest.Name, packageName, packageDir)
	}
	...
	if err := json.Unmarshal(manifest.Bin, &binTarget); err == nil && strings.TrimSpace(binTarget) != "" {
		binTarget = strings.TrimSpace(binTarget)
	} else {
		var binMap map[string]string
		if err := json.Unmarshal(manifest.Bin, &binMap); err != nil || len(binMap) == 0 {
			return "", fmt.Errorf("%w: bin field is malformed in %s", errBinInvalid, packageDir)
		}
		var ok bool
		binTarget, ok = binMap[defaultName]
		if !ok || strings.TrimSpace(binTarget) == "" {
			nonEmptyCount := 0
			...
			if nonEmptyCount != 1 {
				return "", fmt.Errorf("%w: multi-entry bin map without default key %q in %s", errBinInvalid, defaultName, packageDir)
			}
			for _, v := range binMap {
				if strings.TrimSpace(v) != "" {
					binTarget = strings.TrimSpace(v)
					break
				}
			}
		} else {
	...
	resolved := filepath.Join(packageDir, binTarget)
	resolved = filepath.Clean(resolved)
	cleanPkgDir := filepath.Clean(packageDir)
	// Lexical containment check first: reject paths that escape before
	// considering whether the target file exists.
	if rel, err := filepath.Rel(cleanPkgDir, resolved); err != nil || rel == ".." || strings.HasPrefix(rel, ".."+string(os.PathSeparator)) {
		return "", fmt.Errorf("%w: bin target %q escapes package directory %s", errBinInvalid, binTarget, packageDir)
	}
	realPkgDir, pkgErr := filepath.EvalSymlinks(cleanPkgDir)
	if pkgErr != nil {
		return "", fmt.Errorf("%w: cannot resolve real package directory %s: %v", errBinInvalid, cleanPkgDir, pkgErr)
	}
	realResolved, symErr := filepath.EvalSymlinks(resolved)
	if symErr != nil {
		if _, statErr := os.Lstat(resolved); statErr == nil {
			return "", fmt.Errorf("%w: cannot resolve real path for bin target %q: %v", errBinInvalid, resolved, symErr)
		}
		return "", errBinNotInstalled
	}
	if rel, err := filepath.Rel(realPkgDir, realResolved); err != nil || rel == ".." || strings.HasPrefix(rel, ".."+string(os.PathSeparator)) {
		return "", fmt.Errorf("%w: bin target %q escapes package directory %s (via symlink)", errBinInvalid, binTarget, packageDir)
	}
	...
	if !info.Mode().IsRegular() {
		return "", fmt.Errorf("%w: bin target %q is not a regular file", errBinInvalid, realResolved)
	}
	return realResolved, nil
}

var errBinNotInstalled = errors.New("bin not installed")
var errBinInvalid = errors.New("bin invalid")

...
func (s Service) resolveProviderRuntime(ctx context.Context, spec ProviderSpec) providerRuntimeResolution {
	...
	if cliPath == "" && runtime.GOOS == "windows" &&
		spec.Install.Kind == InstallerKindManagedNPMPackage && spec.Install.ManagedNPM != nil {
		nativeExe := s.resolveManagedNPMNativeExe(*spec.Install.ManagedNPM, env)
		if nativeExe != "" {
			cliPath = nativeExe
			if adapterPath == "" {
				adapterPath = nativeExe
			}
		}
	}
...
func shellCommandUsesNPM(command string) bool {
	...
	if runtime.GOOS == "windows" && first == "npm.cmd" {
		return true
	}
	return false
}
...
func (s Service) runExternalAgentRegistryNPMInstaller(ctx context.Context, provider string, spec InstallerSpec) (InstallCommandResult, error) {
	...
	commandArgs := []string{appRuntime.NPM, "--prefix", npmSpec.PrefixDir, "install", packageSpec}
	if runtime.GOOS == "windows" {
		...
		commandArgs = append([]string{appRuntime.Node, npmCLIPath}, npmArgs...)
	}

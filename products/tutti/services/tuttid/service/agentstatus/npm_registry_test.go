...
func TestShellCommandNPMInstallerEnvPrefersManagedRuntime(t *testing.T) {
	...
	if !strings.Contains(strings.Split(path, string(os.PathListSeparator))[0], managedNodeBin) {
		t.Fatalf("PATH = %q, want managed node bin first", path)
	}
}

func TestShellCommandUsesNPM(t *testing.T) {
	t.Parallel()
	...
	cases := map[string]bool{
		...
		"NPM install foo":            true,
		"Npm install foo":            true,
		"/usr/local/bin/npm install": true,
		"npx foo":                    false,
		"node npm-cli.js":            false,
		"pnpm install":               false,
		"":                           false,
		"   ":                        false,
	}
	if runtime.GOOS == "windows" {
		cases["npm.cmd install -g openclaw"] = true
		cases[`C:\tools\npm.cmd install`] = true
	} else {
		cases["npm.cmd install -g openclaw"] = false
		cases[`C:\tools\npm.cmd install`] = false
	}
	for cmd, want := range cases {
		if got := shellCommandUsesNPM(cmd); got != want {
			t.Errorf("shellCommandUsesNPM(%q) = %v, want %v", cmd, got, want)
		}
	}
}

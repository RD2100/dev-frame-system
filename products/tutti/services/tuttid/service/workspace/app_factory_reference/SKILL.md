...
- `bootstrap.sh`: executable shell entrypoint that starts the app server with no arguments.
  - On Windows, the runner looks for a `bootstrap.ps1` sibling beside `bootstrap.sh`.
    Include both files for cross-platform packages; the manifest keeps `"bootstrap": "bootstrap.sh"`.

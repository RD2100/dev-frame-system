...
def validate_scripts(root: Path, manifest: dict[str, Any] | None, errors: list[str]) -> None:
    ...
    bootstrap = runtime.get("bootstrap") if isinstance(runtime, dict) else None
    ...
    if bootstrap and bootstrap.endswith(".sh"):
        ps1_name = bootstrap[:-3] + ".ps1"
        ps1_path = root / ps1_name
        if ps1_path.is_file():
            ps1_text = ps1_path.read_text(encoding="utf-8", errors="ignore")
            for line_number, line in enumerate(ps1_text.splitlines(), start=1):
                stripped = line.strip()
                if not stripped or stripped.startswith("#"):
                    continue
                if BARE_RUNTIME_RE.search(stripped) and "TUTTI_APP_" not in stripped:
                    errors.append(f"{ps1_name}:{line_number} uses bare runtime command; use TUTTI_APP_* variables")
            if re.search(r"\b(install|npm\s+i|npm\s+install|pip\s+install)\b", ps1_text):
                errors.append(f"{ps1_name} should launch only; move install/build work to prepare.sh")
        elif sys.platform == "win32":
            errors.append(
                f"Missing {ps1_name} (Windows PowerShell companion for {bootstrap}). "
                "Cross-platform packages should include bootstrap.ps1 beside the .sh bootstrap. "
                "Use $env:TUTTI_APP_NODE and $env:TUTTI_APP_PACKAGE_DIR."
            )

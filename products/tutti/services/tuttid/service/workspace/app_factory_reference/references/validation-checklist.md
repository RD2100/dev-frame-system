...
- Cross-platform packages include a `bootstrap.ps1` beside `bootstrap.sh` for Windows.
- `bootstrap.ps1` uses `$env:TUTTI_APP_NODE`, `$env:TUTTI_APP_PACKAGE_DIR`, and other `TUTTI_APP_*` environment variables; it performs no startup installation.

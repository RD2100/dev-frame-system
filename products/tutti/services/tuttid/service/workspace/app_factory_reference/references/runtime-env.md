...
The runner starts the app bootstrap script from `TUTTI_APP_RUNTIME_DIR` with no
arguments. On Linux/macOS the runner selects the named bootstrap directly (normally
`bootstrap.sh`). On Windows the runner prefers a `.ps1` sibling when the manifest
names a `.sh` bootstrap; use `$env:TUTTI_APP_*` environment variables in the
PowerShell script.

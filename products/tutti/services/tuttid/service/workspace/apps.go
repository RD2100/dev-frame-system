...
func (s *AppCenterService) InitBuiltinPackages(ctx context.Context) error {
	...
	for _, builtin := range builtins {
		...
		case builtinapps.DistributionEmbeddedArchive:
			if !isBuiltinSupportedOnPlatform(builtin.Distribution, runtime.GOOS, runtime.GOARCH) {
				slog.Warn(
					"embedded builtin app not supported on current platform",
					"appId", builtin.Manifest.AppID,
					"version", builtin.Manifest.Version,
					"platform", runtime.GOOS,
					"arch", runtime.GOARCH,
					"reason", "platform not in distribution platforms list",
				)
				continue
			}

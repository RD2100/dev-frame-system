...
func TestAppCenterServiceInitializesBuiltinCatalogAndInstallState(t *testing.T) {
	if runtime.GOOS != "darwin" {
		t.Skip("embedded builtin catalog test requires darwin (onboarding is darwin-only)")
	}
...
func TestAppCenterServiceInitializesBuiltinPackagesWhenRemoteCatalogFails(t *testing.T) {
	...
	if runtime.GOOS == "darwin" {
		...
		if packages, err := store.ListAppPackages(context.Background()); err != nil {
			...
			t.Fatalf("ListAppPackages() len = %d, want 0 (onboarding is darwin-only)", len(packages))
		}
	}
...
`)
	...
	if err := os.WriteFile(filepath.Join(packageDir, "bootstrap.ps1"), []byte("exit 0\n"), 0o644); err != nil {
		t.Fatalf("write bootstrap.ps1: %v", err)
	}

...
func TestDefaultResolverPreloadsNodeStaticFromLocalRootWithoutCatalog(t *testing.T) {
	catalogValues := []string{
		tuttiAppRuntimeCatalogEnv + "=",
		tuttiAppRuntimeCatalogEnv + "=" + filepath.Join(t.TempDir(), "nonexistent.json"),
	}
	for _, cv := range catalogValues {
		t.Run("catalog="+cv, func(t *testing.T) {
			root := t.TempDir()
			nodeBinDir := filepath.Join(root, "node", "bin")
			...
			resolver := DefaultResolver{
				RuntimeRoot: root,
				Environ: func() []string {
					return []string{cv, "PATH=/usr/bin:/bin"}
				},
			}
			if err := resolver.PreloadProfile(context.Background(), "node-static"); err != nil {
			...
			if !isExecutableFile(resolved.Node) || !isExecutableFile(resolved.NPM) {
				t.Fatalf("ResolveProfile(node-static) did not resolve node runtime: %#v", resolved)
			}
			if resolved.Python != "" {
				t.Fatalf("ResolveProfile(node-static) Python = %q, want empty", resolved.Python)
			}
		})
	}
}

func TestDefaultResolverBaselineStillRequiresPython(t *testing.T) {
	root := t.TempDir()
	nodeBinDir := filepath.Join(root, "node", "bin")
	if err := os.MkdirAll(nodeBinDir, 0o755); err != nil {
		t.Fatalf("mkdir node bin dir: %v", err)
	...
	if err == nil {
		t.Fatal("Resolve() error = nil, want missing python error")
	}
}
